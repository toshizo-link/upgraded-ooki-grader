...
if ($null -eq $installation -or
    -not ([string] $installation.serviceName).Equals(
        $ServiceName,
        [StringComparison]::Ordinal)) {
    throw 'Restore requires the persistent installation manifest and its immutable service identity.'
}
...
if (-not $version.Equals(
    ...
    throw 'Restore must target the exact version recorded in the persistent installation manifest.'
}
