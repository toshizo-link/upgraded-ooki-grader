...
通常のログインは、操作がある限り既定で 30 日のアイドル期限が更新され、
絶対期限は 90 日です。30 日未満のアイドル期限は設定できません。無効化、
パスワード再設定、明示ログアウトは期限を待たずにセッションを終了します。
...
6. 成功後、接続状態が `正常`、モデルが `gemini-3.7-flash`、最終確認時刻が現在であることを確認する。新しいプロファイルの思考レベルは `LOW`、`MEDIUM`、`HIGH` のいずれかとし、`MINIMAL` は使用しない。
...
モデルだけを変更する場合は、既存接続を編集して正確なモデル ID を入力し、
API キー欄を空欄のまま `接続を確認して有効化` を押します。暗号化済みキーを
再利用して同じ能力確認を行うため、キーの再入力は不要です。失敗した場合は
以前のモデルと 4 機能が維持されます。過去の評価記録や確定済み採点のモデル
来歴は変更しません。
...
1. 架空の生徒を 1 名登録する。
2. サンプル PDF から HOP または STEP のひな形を作り、`大問` と `中問` は配点なしの範囲、`中問` は必須、配点は必ず `小問` に設定されることを確認する。複数の正答は完全な別解として登録され、AI 下書きの備考と自由記述の採点基準が空欄であることも確認する。
3. 問題、正答、配点、`完答`、`順不同`、`漢字必須` を確認する。変更が `保存中` から `保存済み` へ自動保存され、個別の保存・確認操作が不要であることと、`この漢字必須設定をすべての問題に適用` を確認する。実施日と必要な場合の対象クラスを指定して `受付を開始` する。試験名・教科・学年・カテゴリ・コースはひな形から引き継がれ、同じ操作で版の確定と答案受付開始が完了する。
4. 開いた `受付中` テストの対象名簿へ架空生徒を追加する。
5. 実施一覧の `受付中テストへ一括仕分け` に複数の 1 ページ PDF をまとめて入れ、プリンター名の自然順、公開済み原稿とのローカル照合による受付先／状態、曖昧時の手動選択、`仕分け結果で受付を開始` 後の 1 ファイルずつの処理を確認する。
6. 生徒名確認後、採点画面の `不正解のみ` で誤答候補を確認・修正し、確定まで進める。
7. 帳票を開き、答案用紙の後ろに簡潔な採点結果が結合された結果 PDF を作成・ダウンロードする。`合否表` が確定結果に追随することを確認し、`合否表をPDF出力` から印刷画面の `PDFとして保存` を試す。
