...
通常の職員セッションは、操作がある限り 30 日のアイドル期限が更新され、
既定の絶対期限は 90 日です。アイドル期限を 30 日未満には設定できません。
それより早い予期しないログアウトは仕様として扱わず、アカウント無効化、
パスワード変更、明示ログアウト、時刻ずれ、Cookie／Origin エラーを調査します。
...
7. モデルが正確に `gemini-3.7-flash`、接続状態が正常、最終接続確認が現在時刻、4 機能がすべて `利用できます` であることを確認します。新しいプロファイルの思考レベルは `LOW`、`MEDIUM`、`HIGH` のみを使用し、`MINIMAL` は使用しません。
...
モデルだけを変更する場合は、既存の Gemini 接続を編集し、モデル ID
だけを書き換えて API キー欄を空欄のまま `接続を確認して有効化` を
押します。ホストは暗号化済みキーを再利用して同じ完全な能力確認を行い、
成功時だけモデルと 4 機能の現行設定を更新します。キーは交換時だけ再入力
します。過去の評価記録、実行中の不変プロファイル、確定済み採点のモデル
来歴は変更しません。
...
DeepSeek V4 Flash（`deepseek/deepseek-v4-flash` と固定版 `deepseek/deepseek-v4-flash-0731`）はテキスト専用です。実画像試験では構造化テキスト応答には成功したものの、画像入力対応エンドポイントがなく拒否されました。現行の画像ワークフローには使用せず、既定の Gemini 3.7 Flash を維持します。詳細は [比較レポート](../../output/accuracy/openrouter-deepseek-v4-vs-gemini-report-2026-08-05.md) を参照してください。
...
8. 元画像と AI 下書きを比較し、設問文、配点、正答が一致すること、AI が不確かな項目は個別確認として残ることを確認する。`大問` と `中問` は配点なしの範囲、`中問` は必須、配点は必ず `小問` に設定されます。複数の別解はそれぞれ完全な正答として登録され、AI 下書きの備考と自由記述の採点基準は空欄です。すべての対応形式で `AIで判定（おすすめ）` が初期選択され、`完答`、`順不同`、`漢字必須` は用紙に合わせて提案されます。変更が自動保存されること、`この漢字必須設定をすべての問題に適用` が他の採点条件を変えないことも確認します。必要な問題だけ完全一致、数値、選択式、先生採点へ変更します。
...
スキャナーが答案を 1 ページずつ別の PDF として出力する場合も、ファイル名に生徒名を付ける必要はありません。実施一覧の `受付中テストへ一括仕分け` が全受付中テスト共通の投入場所です。プリンターのファイル名を保持して自然順に並べ、ホストが各 PDF を受付中テストの公開済み原稿ページとローカルで照合して受付先を提案します。一致が弱い、複数候補が同程度、または原稿を利用できないファイルは勝手に決めず、各行で先生が受付先を選びます。Gemini や汎用 AI へ送らず、仕分けでは AI トークンを使いません。

受付先が決まった後は既存のテスト別順番取込を使います。`受付を開始` したテスト実施は、その時点で確定したひな形版の答案ページ数を使用します。HOP は 1 枚、STEP は選択した登録済みバリエーション／テスト実施ごとに 2 枚です。STEP の `-1`／`-2`／`-3` は別々のテストなので、作成元の 6 ページ一式を 1 人分として取り込みません。クラス分けテストと `その他` は確定したひな形全体のページ数で、1 答案の上限は 50 ページです。`その他` は 3、4、16 ページなどでも同じ処理になります。後から新しいひな形版を確定しても、受付中のテスト実施は開始時の版を使い続けます。

1. 生徒 1 人分の全ページを、1 ページ目から最後のページまで続けてスキャンする。
2. 次の生徒へ移り、同じ順番を繰り返す。複数テストの PDF は共通投入欄へまとめて追加できるが、各テスト内では生徒ごとのページを連続させる。
3. 自然順に並んだ各ファイルの受付先、状態と、受付先ごとの `答案 1: 1/2, 2/2` のような区切りを確認する。曖昧な受付先を選び、必要なら送信前にファイルを削除・追加する。
4. `仕分け結果で受付を開始` を選ぶ。画面の順番どおり 1 ファイルずつ処理され、進捗は該当する `受付中` ボックスへ反映される。テスト別画面から取り込む場合は従来の `この順番でページを送信` を使う。
...
AI が不正解とした箇所だけを確認するときは `不正解のみ` を選びます。
これは表示の絞り込みだけで、点数や確認状態を変更しません。修正後は
`すべて` に戻して、氏名・合計・未確認件数を確認します。
...
ひな形の問題を開き、まず `大問`／`中問`／`小問` を原本どおりに確認
します。`大問` は配点を持たない範囲、`中問` は必須です。`小問` が
なければ中問が採点対象、あれば小問が採点対象で中問は範囲になります。
大問から小問へ直接つなげません。表、図の穴埋め、通常の穴埋め、問答式も
同じ規則ですが、例外的な紙面は原本のまとまりを優先します。

AI が作った下書きでは、備考と自由記述の `採点基準` は空欄です。先生が
必要な場合だけ入力します。別解は断片を一つに連結せず、それぞれ単独で
完全な正答として追加します。そのうえで、採点対象の中問または小問の
`採点の詳細設定` から必要な項目を確認します。
...
同じ漢字方針を全問へ設定する場合は、基準にする問題で
`この漢字必須設定をすべての問題に適用` を使います。`完答`、`順不同`、
正答、採点基準は変更されません。通常の編集は短い待ち時間後に自動保存
され、画面上部が `保存中` から `保存済み` になります。問題ごとに
`保存`／`確認` を押す必要はありません。

公開後の版は変更しません。修正は新しい下書き版で行い、受付開始前に 3 項目と正解・別表記・`漢字必須の例外（読み）`・配点を確認してください。
...
各結果 PDF は、保存期間内の元答案／組立済み答案 PDF を先頭に置き、その後ろに
氏名・生徒番号・学年・クラス・合計点、必要な `大問`／`中問`／`小問`、
生徒解答、登録済みの複数正答、得点をまとめた簡潔な採点結果を付けます。
答案画像が保存期限切れ、または元ファイルが PDF でない場合も、採点結果だけの
PDF を作成できます。備考と先生専用の自由記述採点基準は出力しません。

同じ帳票画面の `合否表` は現在の検索・絞り込みに一致する確定結果から作られ、
合格基準（既定 60%）、合格／不合格件数、生徒×テストの得点を表示します。
結果状態の通知を受けて更新し、通知がない場合も 30 秒ごとに再確認します。
`合否表をPDF出力` を押すと印刷用表示になるので、Edge／Chrome の印刷画面で
`PDFとして保存` を選びます。合否表の印刷は元の確定結果を変更しません。
...
設定、試験専用宛先での確認、停止と後片付けの詳細は、
[個人採点結果の自動配信手順](school-manager-automatic-results-ja.md)を参照してください。
...
決定的ひな形作成への更新では、`_0017_DeterministicTemplateGenerationBatches` を旧スキーマの複製で事前検証します。更新後の起動時に、Gemini の利用中プロファイルが正確な `template-extract-v2.1.0`／`template_extract_v6` と現行ハッシュへ自動調整されたことを確認し、必要なら `接続を確認` を 1 回実行して自己修復させます。別途、匿名の固定サンプルでリリース受入を行います。問題があれば `Features:Ai.TemplateGeneration` を `false` にして新しい生成だけを停止します。アップロード原本、バッチ、生成単位、監査履歴は削除しません。旧バイナリが新スキーマを読めない場合は上書き起動せず、サービス停止後に更新直前の検証済みバックアップをオフライン復元します。旧作成画面を再公開したり、失敗した HOP／STEP の一部だけを手作業で確定したりしません。
...
1. `管理 > AI設定` で既定の Gemini モデルが `gemini-3.7-flash` か。OpenRouter を追加した場合は、入力した正確なモデル ID か。
...
- [ ] `接続を確認して有効化` が成功し、モデルが `gemini-3.7-flash`、最終確認時刻が現在になった
...
- [ ] 起動後または `接続を確認` 後、ひな形作成が正確な `template-extract-v2.1.0`／`template_extract_v6` と現行ハッシュへ自動調整されている
