...
8. 元画像と AI 下書きを比較し、設問文、配点、正答が一致すること、AI が不確かな項目は個別確認として残ることを確認する。`大問` は配点なしの範囲、`中問` は必須、`小問` があれば小問に配点され、なければ中問に配点されます。複数の別解はそれぞれ完全な正答として登録され、AI 下書きの備考と自由記述の採点基準は空欄です。すべての対応形式で `AIで判定（おすすめ）` が初期選択され、`完答`、`順不同`、`漢字必須` は用紙に合わせて提案されます。変更が自動保存されること、`この漢字必須設定をすべての問題に適用` が他の採点条件を変えないことも確認します。必要な問題だけ完全一致、数値、選択式、先生採点へ変更します。
