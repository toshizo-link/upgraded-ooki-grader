...
$expectedArchiveSha256 = 'aae10a4ec12d9c9c8ce9df622554d8ae1dbc45b527ba7e8ca8d5440ac2ced11f'
...
$transcriptPath = Join-Path $diagnosticRoot (
    "host-update-$version-$runId.log")
$resultPath = Join-Path $diagnosticRoot (
    "host-update-$version-$runId.result.json")
...
function Write-DiagnosticResult {
    ...
    $result = [ordered]@{
        schema = 'ooki-host-update-run/v1'
        state = $State
        version = $version
...
trap {
    ...
    [Console]::Error.WriteLine("Update failed: $failureMessage")
...
if ([string] $installation.version -eq '0.9.11') {
    Write-Host 'Ooki Grader is already at version 0.9.11.'
