...
$expectedArchiveSha256 = 'be212dd99a614047217f53cec5410ee51149634b43f9fa19735f2848d10f9c15'
...
if ([string] $installation.version -eq '0.9.3') {
    Write-Host 'Ooki Grader is already at version 0.9.3.'
...
if ($health.database.state -ne 'healthy' -or
    ...
    -not [bool] $health.database.maintenanceMode -or
    [bool] $health.storage.restoreOrMigrationMarkerPresent) {
    throw 'The host must be healthy, schema-current, in maintenance mode, and free of restore/migration markers.'
}

Write-Host ''
Write-Host 'Ooki Grader portable update'
Write-Host "  Current version: $($installation.version)"
Write-Host '  New version:     0.9.3'
...
Write-Host '  Safety:          maintenance mode, verified backup, prior version retained'
Write-Host ''
