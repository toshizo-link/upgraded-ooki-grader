...
$expectedArchiveSha256 = '296a3c674376a53ce771a33ae5ed2367542f8bddb9ac2b0a6f4aed20b0938571'
...
if ([string] $installation.version -eq '0.9.4') {
    Write-Host 'Ooki Grader is already at version 0.9.4.'
...
if ([string]::IsNullOrWhiteSpace($backupRoot) -or
    ...
    throw 'A configured backup destination is required before updating.'
}
if (-not [bool] $configuration.Backup.DestinationEncryptionConfirmed) {
    throw 'The configured backup destination is not confirmed as encrypted.'
}
...
if ($null -eq $latestManifestFile) {
    throw 'No completed backup manifest was found in the configured destination.'
}
try {
    $backupManifest = Get-Content -LiteralPath $latestManifestFile.FullName `
        -Raw | ConvertFrom-Json
} catch {
    throw 'The newest backup manifest is invalid.'
}
$backupId = [string] $backupManifest.backupId
if ($backupId -notmatch '^[0-9A-HJKMNP-TV-Z]{26}$') {
    throw 'The newest backup manifest has an invalid backup identifier.'
}
if (-not ([string] $backupManifest.applicationVersion).Equals(
    [string] $installation.version,
    [StringComparison]::Ordinal)) {
    throw 'The newest backup was not created by the currently installed version.'
}
if (-not [bool] $backupManifest.destinationEncryptionConfirmed) {
    throw 'The newest backup does not confirm destination encryption.'
}
...
if ($backupCompletedAt -lt [DateTimeOffset]::UtcNow.AddHours(-24)) {
    throw 'The newest backup is more than 24 hours old. Create and verify a fresh backup before updating.'
}
$backupPrefix = [IO.Path]::GetFullPath($backupRoot).TrimEnd('\') + '\'
$backupSetPath = [IO.Path]::GetFullPath($latestManifestFile.DirectoryName)
if (-not $backupSetPath.StartsWith(
    $backupPrefix,
    [StringComparison]::OrdinalIgnoreCase)) {
    throw 'The newest backup manifest is outside the configured backup root.'
}
$backupRelativePath = $backupSetPath.Substring($backupPrefix.Length).Replace(
    '\',
    '/')
$backupManifestSha256 = Get-Sha256 -Path $latestManifestFile.FullName
...
Write-Host '  New version:     0.9.4'
Write-Host "  Data:            $dataRoot"
Write-Host "  Backup:          $backupRelativePath"
...
Write-Host ''
$confirmation = Read-Host 'Type UPDATE to install this update'
